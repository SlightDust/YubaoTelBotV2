...
async def setu(chat:extendChat, match):
    ...
    for image in images:
        ...
        if ("R-18" in tags) and ("private" not in chat.type):
            ...
            attext, at = await at_sender(chat)
            try:
                ...
                await chat.send_text(f'{attext} R-18图片已通过私聊发送。\n任何人点击按钮可接收私聊消息。', reply_markup=json.dumps(inlinekeyboardmarkup), entities=at)
            except BotApiError as e:    # 未认证bot
                print(e.args)
                if "bot can't initiate conversation with a user" in str(e.args):
                    await chat.reply(f'R-18图片私聊发送失败。请先私聊域宝并发送/start后再试。')
                elif "bot was blocked by the user" in str(e.args): 
                    await chat.reply(f'您屏蔽了域宝的私聊消息。R-18图片发送失败。')
                else:
                    await chat.reply(f'R-18图片私聊发送失败: \n{e}')
                pass
        else:
...
async def showr18pic(chat, cq, match):
    ...
    attext, at = await at_callbacker(cq)
    # print(chat_id, from_chat_id, message_id)
    # print(cq.src)
    try:
        await bot.copy_message(chat_id, from_chat_id, message_id)
        await bot.send_message(chat_id=cq.src.get('message').get('chat').get('id'),text=f"{attext} 您点击了按钮，私聊请查收~", reply_to_message_id=cq.src.get('message').get('message_id'), entities=at)
    except BotApiError as e:    # 未认证bot
        print(e.args)
        if "bot can't initiate conversation with a user" in str(e.args):
            await bot.send_message(chat_id=cq.src.get('message').get('chat').get('id'), text=f'{attext} R-18图片私聊发送失败。请先私聊域宝并发送/start后再试。', entities=at)
        elif "bot was blocked by the user" in str(e.args): 
            await bot.send_message(chat_id=cq.src.get('message').get('chat').get('id'), text=f'{attext} 您屏蔽了域宝的私聊消息。R-18图片发送失败。', entities=at)
        else:
            await bot.send_message(chat_id=cq.src.get('message').get('chat').get('id'), text=f'{attext} R-18图片私聊发送失败: \n{e}', entities=at)
