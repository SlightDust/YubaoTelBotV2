...
async def get_rss_news(rss_url):
    ...
    for item in feed["entries"]:
        if get_published_time(item) > last_time:
            logger.info(f'{item["title"]}：{get_published_time(item)}>{last_time}')
...
async def get_enable_groups():
    path = os.path.join(os.path.dirname(__file__), 'rssgroups.json')
    if not os.path.exists(path):
        with open(path, 'w', encoding='utf8') as f:
...
async def group_process():
    bot = yubao.get_bot()
    groups = await get_enable_groups()
    logger.info("开始刷新rss")
...
@bot.command(r"rss (.+)")
async def rss(chat:Chat, match):  # 需要修改为插件名
    ...
    msg = ''
    group_id = chat.id
    args = match.group(0).replace("rss","").split()
    print(args)
    is_admin = True
    if len(args) == 0:
        msg = HELP_MSG
